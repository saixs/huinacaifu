...
class linksClass {
	...
	function GetList($data = array()){
		...
		if (isset($data['limit']) ){
			...
			return $mysql->db_fetch_arrays(str_replace(array('SELECT', 'ORDER', 'LIMIT'), array($_select,  'order by p1.`order` desc,p1.id ASC', $_limit), $sql));
		}	
		...
		$list = $mysql->db_fetch_arrays(str_replace(array('SELECT', 'ORDER', 'LIMIT'), array($_select, 'order by p1.id asc', $limit), $sql));
