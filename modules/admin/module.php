 ...
 **/
    ...
    if (!file_exists(ROOT_PATH . "/modules/$code/$code.php")) {
        ...
        $_check = $_A['query_class'].'_all';
        if (
            $code != "manager" && $code != "user" && isset($_A['site_result']['rank'])
            &&
            !in_array($_G['user_result']['type_id'], explode(",", $_A['site_result']['rank']))
            &&
            $_G['user_result']['type_id'] != 1
            &&
            !in_array("other_all", explode(",", $_SESSION['purview']))
            &&
            (
                !(
                    $_A['query_class'] == 'article'
                    &&
                    in_array( "content_all", explode(",", $_SESSION['purview']) )
                )
                &&
                !in_array( "content_all", explode(",", $_SESSION['purview']) )
            )
            &&
            ($_A['query_class'] != 'borrow' && $_SESSION['purview'] != 'borrow_all' && $_SESSION['logintype'] != 'admin')
        )
        {
