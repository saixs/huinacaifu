...
elseif ($_A['query_type'] == "type_new" || $_A['query_type'] == "type_edit"){
	if (isset($_POST['name'])){
		...
		$_purview = array(
            "other"=>array("特殊权限"=>array("other_all"=>"所有权限","site_all"=>"站点管理","module_all"=>"模块管理","system_all"=>"系统管理","bbs_all"=>"论坛管理")),
        ...
        $_res = $user->GetTypeOne($data);
        if (!empty($_res)) {
            $pur = explode(',', $_res['purview']);
        } else {
            $pur = array();
        }

        foreach ($_pur as $key => $value) {
            if (in_array($key,$pur)) {
                $_pur[$key] = true;
            } else {
                $_pur[$key] = false;
