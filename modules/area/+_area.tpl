...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center" width="130">{if $magic.request.action=='' || $magic.request.action=='province'}<a href="{$_A.query_url}&action=city&pid={$item.id}">城市</a> /{elseif $magic.request.action=='city'} <a href="{$_A.query_url}&action=area&pid={$item.id}">地区</a> / {/if}<a href="{$_A.query_url}/edit&action={ $magic.request.action|default:'province'}&pid={$item.pid}&id={$item.id}">修改</a> / <a href="#" onClick="javascript:if(confirm('确定要删除吗?删除后将不可恢复')) location.href='{$_A.query_url}/del&id={$item.id}'">删除</a></td>
	</tr>
	{ /foreach}
	<tr >
