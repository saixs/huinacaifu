...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	<tr>
		...
		<td>
			<a href="{$_A.admin_url}&q=module/attestation/bankCheck&type=allow&id={$var.id}">通过</a> |
			<a href="{$_A.admin_url}&q=module/attestation/bankCheck&type=reject&id={$var.id}">拒绝</a>
		</td>
	</tr>
	{/foreach}
</table>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<form action="" method="post">
		...
		<td class="main_td1" align="center" >
			{if $item.status !=1}<a href="{$_A.query_url}/view{$_A.site_url}&id={$item.id}">[审核]</a>{/if}
			{if $item.is_delete !=1}<a href="{$_A.query_url}/view{$_A.site_url}&del_id={$item.id}">[删除]</a>{/if}
		</td>
		</tr>
		{ /foreach}
		<tr>
...
<div class="module_add">
	...
	<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
		...
		<td class="main_td1" align="center" >
			{if $item.phone_status>1}<font color="#999999">[手机：{$item.phone}]</font>
			<a href="javascript:void(0)" onclick='tipsWindown("手机认证审核","url:get?{$_A.query_url}/audit&user_id={$item.user_id}&type=phone",500,230,"true","","true","text");'>
				<font color="#FF0000">等待审核</font>
			</a>
			{elseif $item.phone_status==1}
			<font color="#999999">[手机：{$item.phone}]</font>
		...
		<tr>
			<td colspan="11" class="action">
				...
				<div class="floatr">
					用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/> 真实姓名：<input type="text" name="realname" id="realname" value="{$magic.request.realname|urldecode}"/> 状态<select id="type" ><option value="">全部</option>
	...
	<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
		...
		<tr>
			<td colspan="10" class="action">
				...
				<div class="floatr">
					用户名：<input type="text" name="keywords" id="keywords" value="{$magic.request.keywords|urldecode}"/> 姓名：<input type="text" name="realname" id="realname" value="{$magic.request.realname|urldecode}"/>
	...
	<div class="module_add">
		...
		<table width="1000" border="1">
			...
			<tr>
				...
				<td  ><pre>{$item.credit|credit}{$item.credit}分</pre></td>
				<td  ><pre>￥{$item.account}</pre></td>
				<td  ><pre>{$item.apr}%</pre></td>
				<td  ><pre>{ if $item.isday ==1}{$item.time_limit_day}天{ else}{$item.time_limit}个月{/if}</pre></td>
		...
		<table width="1000" border="1">
			...
			<td  ><pre>{if $item.is_vouch==1}担保标{else}普通标{/if}</pre></td>
			<td  ><pre>{$item.account}元</pre></td>
			<td  ><pre>{$item.apr} %</pre></td>
			<td  ><pre>{ if $item.isday ==1}{$item.time_limit_day}天{ else}{$item.time_limit}个月{/if}</pre></td>
			...
			<td  ><pre>{if $item.status==0}发布审批中{elseif $item.status==1}{if $item.account_yes==$item.account}满标审核中{else}正在募集{/if}{elseif $item.status==2}审核失败{elseif $item.status==3}已满标{elseif $item.status==4}满标审核失败{elseif $item.status==5}撤回{/if}</pre></td>
