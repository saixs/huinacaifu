...
elseif ($_A['query_type'] == 'bankCheck') {
    if ( isset($_GET['type']) && $_GET['type'] == 'allow' ) {
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
        $sql = "UPDATE {account_bank} SET `status`=1 WHERE `id`={$id}";
        $result = $mysql->db_query($sql);
        if ($result) {
            $msg = array('恭喜,操作成功','','http://www.bljinrong.com/index.php?cmdl&q=module/attestation/bankCheck');
        } else {
            $msg = array('抱歉,发生未知错误,请与管理员联系','','http://www.bljinrong.com/index.php?cmdl&q=module/attestation/bankCheck');
        }
    } elseif ( isset($_GET['type']) && $_GET['type'] == 'reject' ) {
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
        $sql = "UPDATE {account_bank} SET `status`=2 WHERE `id`={$id}";
        $result = $mysql->db_query($sql);
        if ($result) {
            $msg = array('恭喜,操作成功','','http://www.bljinrong.com/index.php?cmdl&q=module/attestation/bankCheck');
        } else {
        ...
        $sql = "SELECT acc.*,usr.username,usr.realname,usr.email_status,usr.phone_status,usr.email,usr.phone
                FROM {account_bank} as acc
                LEFT JOIN {user} as usr
                ON acc.`user_id`=usr.`user_id`
                WHERE acc.`status`=0";
	...
	if (isset($_POST['id'])){
        ...
        if (is_array($res)) {
            if (is_file($_SERVER['DOCUMENT_ROOT'].'/'.$res['litpic']) && unlink($_SERVER['DOCUMENT_ROOT'].'/'.$res['litpic'])) {
                ...
                $msg = array("文件无法删除,该文件不存在请与系统管理员取得联系.");
            }
        } else {
            $msg = array("操作失败,不存在该图片信息.");
        }
    }
    else
    {
				...
				if ($_POST['nid']=="realname"){
					...
					if ($user_result['video_status']==1){
						...
						if ($_G['system']["con_video_feestatus"]==1){
							...
							$log['user_id'] = $_data['user_id'];
							$log['type'] = "video";
							$log['money'] = $_G["system"]["con_video_fee"];
							$log['total'] = $account_result['total']-$log['money'];
							$log['use_money'] = $account_result['use_money']-$log['money'];
							$log['no_use_money'] = $account_result['no_use_money'];
							$log['collection'] = $account_result['collection'];
...
elseif ($_A['query_type'] == "vip"){
	...
	if (isset($_REQUEST['kefu']) && $_REQUEST['kefu']!=""){
		...
		$data['kefu_username'] = iconv("UTF-8","GB2312",$data['kefu']);
	}
		...
		if($result['vip_status']==1){
			...
			if ($data['vip_status']==1){
				...
				//$sqls="update `{user_amount}` set credit=credit+50000,credit_use=credit_use+50000,borrow_vouch=borrow_vouch+50000,borrow_vouch_use=borrow_vouch_use+50000 where user_id='$user_id'";
					$sqls="update `{user_amount}` set credit=credit+0,credit_use=credit_use+0,borrow_vouch=borrow_vouch+0,borrow_vouch_use=borrow_vouch_use+0 where user_id='$user_id'";
