...
class attestationClass{
	...
	function GetList($data = array()){
		...
		$_sql = "where 1=1 and is_delete=0";
		...
		if (isset($data['limit']) && $data['limit']!="" ){
			...
			$list = $mysql->db_fetch_arrays(str_replace(array('SELECT', 'ORDER', 'LIMIT'), array($_select, 'order by p1.`id` asc', $_limit), $sql));
            return $list;
		}
		...
		$list = $mysql->db_fetch_arrays(str_replace(array('SELECT', 'ORDER', 'LIMIT'), array($_select, 'order by  p1.status asc,p1.id asc', $limit), $sql));		
