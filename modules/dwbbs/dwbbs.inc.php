...
elseif ($q == "view"){
	...
	if($result['islock']==1 && !dwbbsClass::isForumAdmin(array("fid"=>$result['fid']))){
		$_G['msg'] = array("此帖子已关闭或者已经删除","<a href='/bbs/main.html'>返回论坛首页</a>");
	}else{
...
elseif ($q == "reply"){
	...
	if (isset($_POST['pid']) && $_POST['pid']!=""){
		...
		$_G['msg'] = array("您已成功修改了帖子",'<img src="'.$_G['tpldir'].'/images/bbs_ico_msgp1.gif" border="0" align="absmiddle" alt="" /> <a href="/bbs/main.html?q=forums&fid='.$fid.'"><u>返回帖子列表</u></a><br /><img src="'.$_G['tpldir'].'/images/bbs_ico_msgp1.gif" border="0" align="absmiddle" alt="" /> <a href="/bbs/main.html?q=view&tid='.$tid.'"><u>返回查看帖子</u></a>');
...
";
	...
	if (isset($_REQUEST['fid']) && $_REQUEST['fid']>0){
		...
		if ($_G['bbs_forum_result']!=false){
			...
			$_G['msg'] = array("请不要乱做操作","返回首页","/bbs/main.html");
		}
	}
