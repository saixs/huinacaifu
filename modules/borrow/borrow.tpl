...
<span >借款金额应在500元至50,000元之间。交易币种均为人民币。</span>
	...
	<div class="module_border">
		...
		<div class="c">
			{if  $_A.borrow_result.pwd !=""}是{else}否{/if}
		</div>
	</div>
	<div class="module_border">
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			...
			<td title="{$item.name}">{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}{if $item.is_mb == 1}<span style="color:#FF0000">(秒标)</span>{/if}{if $item.is_jin == 1}<span style="color:#FF0000">(净值标)</span>{/if}{if $item.is_fast == 1}(<a href="javascript:void(0)" onclick='tipsWindown("质押标详细信息","url:get?/index.php?q=viewfast&id={$item.fastid}",500,430,"true","","true","text");'>	<span style="color:#FF0000">{if $item.isqiye == 1}企业-{else}个人-{/if}质押标</span></a>){/if}{$item.name|truncate:10}{if $item.cz_old_id > 0}<span style="color:#FF0000">重组父标：{$item.cz_old_id|czf}</span>{/if}</td>
			...
			<td><select name="flag[]"><option value="0" {if $item.flag==0} selected="selected"{/if}>一般标</option><option  value="1" {if $item.flag==1} selected="selected"{/if}>推荐标</option><!--<option value="2" {if $item.flag==2} selected="selected"{/if}>担保标</option><option  value="3" {if $item.flag==3} selected="selected"{/if}>抵押标</option>--></select></td>
			...
			<td>{ if $item.status ==0 }<a href="{$_A.query_url}/view{$_A.site_url}&user_id={$item.user_id}&id={$item.id}">审核</a>{/if} <a href="{$_A.query_url}/edit{$_A.site_url}&user_id={$item.user_id}&id={$item.id}">查看</a> <a href="#" onclick="javascript:if(confirm('确定要删除吗?删除后将不可恢复')) location.href='{$_A.query_url}/cancel{$_A.site_url}&id={$item.id}'">撤回</a></td>
                        <td><a href={$_A.query_url}/investlist&borrow_id={$item.id}&a=borrow>投标记录</a></td>
		</tr>
		{ /foreach}
		<tr>
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/> 状态<select id="status" ><option value="">全部</option><option value="1" {if $magic.request.status==1} selected="selected"{/if}>已通过</option><option value="0" {if $magic.request.status=="0"} selected="selected"{/if}>待审核</option></select> <select id="is_vouch" ><option value="">全部</option><option value="1" {if $magic.request.is_vouch==1} selected="selected"{/if}>担保标</option><option value="0" {if $magic.request.is_vouch=="0"} selected="selected"{/if}>普通标</option></select> <input type="button" value="搜索"  onclick="sousuo('{$_A.query_url}{$_A.site_url}')"/>
		</div>
		</td>
	</tr>
		<tr>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			...
			<td>{if $item.status!=3}{if $item.is_liuzhuan==1}流转标已满{else}<a href="{$_A.query_url}/full_view{$_A.site_url}&user_id={$item.user_id}&id={$item.id}">审核</a>{/if}{/if}</td>
		</tr>
		{ /foreach}
		<tr>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			<td>{ $item.id}</td>
			<td class="main_td1" align="center"><a href="javascript:void(0)" onclick='tipsWindown("用户详细信息查看","url:get?{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene",500,230,"true","","true","text");'>	{$item.username}</a></td>
			<td title="{$item.borrow_name}">{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}<a href="/invest/a{$item.borrow_id}.html" target="_blank">{$item.borrow_name|truncate:10}</a></td>
			<td>{$item.order+1 }/{$item.time_limit }</td>
			<td>{$item.repayment_time|date_format:"Y-m-d"}</td>
			<td>{$item.repayment_account  }元</td>
            <td>{$item.interest}元</td>
            <td>{$item.late_interest}元</td>
			<td>{$item.repayment_yestime|date_format:"Y-m-d"}</td>
			<td>{if $item.status==1}<font color="#006600">已还</font>{else}<font color="#FF0000">未还--{if $item.status==2}<font color="#FF0000">已代还</font>{else}{if $item.late_days>-5}<a href="{$_A.query_url}/late_repay{$_A.site_url}&id={$item.id}">代还款</a>{else}-{/if}{/if}</font>{/if}</td>
		</tr>
		{ /foreach}
		<tr>
		<td colspan="10" class="action">
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/>关键字：
			<input type="text" name="keywords" id="keywords" value="{$magic.request.keywords|urldecode}"/><select id="status" >
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			...
			<td title="{$item.borrow_name}">{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}<a href="/invest/a{$item.id}.html" target="_blank">{$item.name|truncate:10}</a></td>
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username}"/>关键字：<input type="text" name="keywords" id="keywords" value="{$magic.request.keywords}"/>
			<!--<select id="status" >
			<option value="">不限</option>
			<option value="1" {if $magic.request.status==1} selected="selected"{/if}>已还</option>
			<option value="0" {if $magic.request.status==0} selected="selected"{/if}>未还</option>
...
<div class="module_title"><strong>流标管理</strong></div>
	...
	<div class="module_border">
		<div class="l">标题：</div>
		<div >
			{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}<a href="/invest/a{$_A.borrow_result.id}.html" target="_blank">{$_A.borrow_result.name}</a>
		</div>
	</div>
	<div class="module_border">
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			<td >{ $item.id}</td>
			<td >{ $item.username}</td>
			<td>{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}{if $item.is_fast == 1}(<a href="javascript:void(0)" onclick='tipsWindown("质押标详细信息","url:get?/index.php?q=viewfast&id={$item.fastid}",500,430,"true","","true","text");'>	<span style="color:#FF0000">{if $item.isqiye == 1}企业-{else}个人-{/if}质押标</span></a>){/if}<a href="/invest/a{$item.borrow_id}.html" target="_blank">{$item.borrow_name}</a>{if $item.is_cz==1}  <span style="color:#FF0000">已重组：{$item.borrow_id|czname}</span>{/if}</td>
			<td>{$item.order+1 }/{$item.time_limit}</td>
			<td >{$item.repayment_time|date_format:"Y-m-d"}</td>
			<td >{$item.repayment_account }元</td>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			<td >{ $item.id}</td>
			<td class="main_td1" align="center"><a href="javascript:void(0)" onclick='tipsWindown("用户详细信息查看","url:get?{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene",500,230,"true","","true","text");'>	{$item.username}</a></td>
			<td>{if $item.is_liuzhuan == 1}<span style="color:#FF0000">(流转标)</span>{/if}{if $item.is_fast == 1}(<a href="javascript:void(0)" onclick='tipsWindown("快捷标详细信息","url:get?/index.php?q=viewfast&id={$item.fastid}",500,430,"true","","true","text");'>	<span style="color:#FF0000">{if $item.isqiye == 1}企业-{else}个人-{/if}快捷标</span></a>){/if}<a href="/invest/a{$item.borrow_id}.html" target="_blank">{$item.borrow_name}</a></td>
			<td>{$item.order+1 }/{$item.time_limit}</td>
			<td >{$item.repayment_time|date_format:"Y-m-d"}</td>
			<td >{$item.repayment_account }元</td>
			<td >{$item.late_days}天</td>
...
{ foreach  from=$_A.borrow_invest key=key item=item}
<tr>
<td>{$item.username}</td>
<td>{$item.realname}</td>
<td>{$item.id}</td>
<td>{$item.account}</td>
<td>{$item.name}</td>
<td>{$item.success_time|date_format:"Y-m-d"}</td>
</tr>
{/foreach}
<tr>
		<td colspan="11" class="action">
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/>
                        ...
                        <input type="button" value="搜索" / onclick="sousuo('{$_A.admin_url}&q=module/borrow/invest&site_id=69&a=userinfo')">
		</div>
		</td>
	</tr>
		<tr>
...
<div class="content">
		<table  border="0"  cellspacing="0"  width="100%" class="tab1">
			...
			{if $_A.borrow_investlist==null}
			<tr>
			  <td colspan=7 align="center">暂无投标记录</td>
			 
			</tr>
			{/if}
			{foreach  from=$_A.borrow_investlist key=key item=vat }
			<tr>
			  <td align="center">{$vat.username}</td>
			  <td align="center" >{$vat.apr}%</td>
			  <td align="center" >{$vat.money}元</td>
			  <td align="center" >{$vat.tender_account}元</td>
...
<form action="" method="post">
			...
			{foreach  from=$_A.borrow_tbjl key=key item=vat }
			<tr  >
			  <td align="center">{$vat.username}</td>
			  <td align="center" >{$vat.realname}</td>
			  <td align="center" >{$vat.money}元</td>
			  <td align="center" >{$vat.addtime|date_format:"Y-m-d H:i:s"}</td>
			  <td align="center">{$vat.jl}</td>
			</tr>
...
<tr>
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/>
                        真实姓名：<input type="text" name="realname" id="realname" value="{$magic.request.realname|urldecode}"/>
                        <input type="button" value="搜索" / onclick="sousuo('{$_A.admin_url}&q=module/borrow/tbjl&site_id=69&a=userinfo')">
		</div>
		</td>
	</tr>
		<tr>
...
<div class="module_title"><strong>额度审核</strong></div>
	...
	{if $_A.borrow_amount_result.status!=1}
	<div class="module_title"><strong>审核</strong></div>
	<form method="post" action="">
	<div class="module_border">
		<div class="l">审核状态：</div>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
        ...
        <tr  class="tr2">
			<td >账户总额</td>
			<td >￥{$_A.allaccount_tongji.atotal|default:0.00}</td>
		</tr>
		<tr  >
			<td >可用余额</td>
			<td >￥{$_A.allaccount_tongji.ause_money|default:0.00}</td>
		</tr>
		<tr  class="tr2">
			<td >冻结总额</td>
			<td >￥{$_A.allaccount_tongji.ano_use_money|default:0.00}</td>
		</tr>
		<tr  class="tr2">
			<td >成功借出总额</td>
			<td >￥{$_A.borrow_tongji.success_num|default:0.00}</td>
		</tr>
		<tr  >
			<td >己还款总额</td>
			<td >￥{$_A.borrow_tongji.success_num1|default:0.00}</td>
		</tr>
		<tr  class="tr2">
			<td >未还款总额</td>
			<td >￥{$_A.borrow_tongji.success_num0|default:0.00}</td>
		</tr>
		<tr  >
			<td >逾期总额</td>
			<td >{$_A.borrow_tongji.laterepay|default:0.00}</td>
		</tr>
		<tr  class="tr2">
			<td >逾期己还款总额</td>
			<td >￥{$_A.borrow_tongji.success_laterepay|default:0.00}</td>
		</tr>
		<tr >
			<td >逾期未还款总额</td>
			<td >￥{$_A.borrow_tongji.false_laterepay|default:0.00}</td>
...
var urls = '{$_A.query_url}{$_A.site_url}';
var urlFullExport = "{$_A.query_url}/full&status={$magic.request.status}&type=excel";
var urlFullExport2 = "{$_A.query_url}/investlist&type=excel";
{literal}
