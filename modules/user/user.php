...
if ($_A['query_type'] == "list"){
	...
	$data['email'] = urldecode(isset($_REQUEST['email'])?$_REQUEST['email']:"");
	...
	$result = userClass::GetList($data);
        ...
        if (isset($_REQUEST['type']) && $_REQUEST['type'] == "excel") {
            $title = array("序号", "ID", "用户名", "真实姓名", "性别", "邮箱", "qq", "手机", "所在地", "身份证", "添加时间", "状态", "用户类型");
            $data['limit'] = "all";
            ...
            foreach ($result as $key => $value) {
                ...
                if ($area == "") {
                    ...
                    $sql = "select p1.name as province, p2.name as city, p3.name as area from `{area}` as p1, `{area}` as p2, `{area}` as p3 where p3.pid = p2.id and p2.pid = p1.id and p3.id = $area";
                    $areas = $mysql->db_fetch_array($sql);
                    $value['area'] = $areas['province'] . "|" . $areas['city'] . "|" . $areas['area'];
                }


                $_data[$key] = array($key + 1, $value['user_id'], $value['username'], $value['realname'], $value['sex'], $value['email'], $value['qq'], $value['phone'], $value['area'], $value['card_id'], date('Y-m-d H:i:s', $value['addtime']), $value['status'], $value['typename']);
            }
            exportData("用户列表", $title, $_data);
            exit;
        }
    } else {
        $msg = array($result);
