...
var PopupLayer = new Class({
	...
	_init:function(options){
		...
		this.trigger.bind(this.options.eventType,function(){            //给触发元素绑定触发事件
			if(this.isSetPosition){
				if(this.isCenter){
					var x = $(document).width()/2 -200;// $(this.popupBlk).width();
					//var y = window.screen.availHeight/2 - 200;//$(this.popupBlk).height())/2;
                    //var y=$(window).height()/2;
                    //var y= (document.documentElement.scrollTop + document.body.offsetHeight)/2 - $(this.popupBlk).height() - 200; 
                    var y= document.documentElement.scrollTop + window.screen.availHeight/2 - $(this.popupBlk).height() - 200; 
	...
	setPosition:function(left,top){          //通过传入的参数值改变弹出层的位置
		...
		if(this.isCenter)
		{
//			alert("document width:"+$(document).width()+",document.documentElement.clientHeight:"+ document.documentElement.clientHeight);
//			alert("document.body.clientWidth:"+document.body.clientWidth+",$(document).width():"+$(document).width());
//alert("left:"+left+";top"+top);
//			this.popupLayer.css({"position": "absolute"});
		}
	},
	doEffects:function(way){                //做特效
		way == "open"?this.popupLayer.show("fast"):this.popupLayer.hide("fast");
