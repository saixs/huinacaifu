...
function checkSchool( school ){
	var submit_disabled = false;
    if ( school.length==0 ){
        document.getElementById('school_notice').innerHTML = school_msg;
		submit_disabled = true;
    }
    else{
        document.getElementById('school_notice').innerHTML = msg_can_reg;
    }
...
//检验注册
function userReg(){
    if (checkEmail($("#email").val())  && checkUsername($("#username").val()) && checkPassword($("#password").val())&&checkConformPassword($("#conform_password").val())  && checkRealname($("#realname").val())  &&  checkSchool($("#school").val()) && checkRuxue($("#start_university").val())){
		return true;
	}
   	return false;
}
