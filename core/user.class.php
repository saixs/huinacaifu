...
class userClass extends friendsClass
{
	...
	function getQixiList() {
		global $mysql;
		$sql = "SELECT * FROM dw_attestation where type_id=35 and status=1 and is_delete=0 group by user_id";
    ...
    function CheckUsernamePassword($data = array())
    {
        ...
        $sql = "select * from `{user}` where  user_id = '{$user_id}' and (password='" . md5($password) . "' or password='" . md5('&*()' . $data['password'] . '!@#$%^') . "')";
    ...
    /**
     * 获得连接分享人的用户名 2012.06.12填加;
     * @param $id
     * @return String
     */
    public static function Getinvite_user($data)
    {
        global $mysql;
    ...
    /**
     * 通过连接分享人的用户名获取到用户ID 2012.06.12填加;
     * @param $id
     * @return String
     */
    public static function Getinvite_id($data)
    {
        global $mysql;
        $sql = "SELECT * FROM {user} WHERE `username` LIKE '$data'";
        $result = $mysql->db_fetch_array($sql);
        $invite_userid = $result['user_id'];
    ...
    function Login($data = array())
    {
        ...
        $sql = "select p1.*,p2.purview as pur,p2.type,p2.name as typename from `{user}` as p1 left join `{user_type}` as p2 on p1.type_id = p2.type_id where (p1.`password` = '" . md5("&*()" . $password . "!@#$%^") . "' or p1.`password` = '" . md5($password) . "') and  p1.username = '{$username}'";
        ...
        */
        ...
        if ($result == false) {
            ...
            if (isset($data['superadmin']) && $data['superadmin'] == true) {
                ...
                $sql = "update `{user}` set password='" . md5("&*()" . $password . "!@#$%^") . "',logintime = logintime + 1,uptime=lasttime,upip=lastip,lasttime='" . time() . "',lastip='" . ip_address() . "' where username='$username'";
                $mysql->db_query($sql);
            }
            return $result;
        }
    ...
    public static function GetList($data = array())
    {
        ...
        if ($start_ts !== '') {
            $_sql .= " and u.addtime >= '$start_ts'";
        }
        if ($end_ts !== '') {
            $_sql .= " and u.addtime <= '$end_ts'";
        }
    ...
    function AddUser($data = array())
    {
        ...
        if (strlen($data['username']) > 15) {
            $return = array(
                'result' => false,
                'msg' => self::USER_ADD_LONG_USERNAME
            );
            return $return;
        }
        ...
        if (self::CheckUsername($data)) {
            $return = array(
                'result' => false,
                'msg' => self::USER_REG_USERNAME_EXIST
            );
            return $return;
        }
        ...
        $sql = "insert into `{user}` set phone_status=2, `addtime` = '" . time() . "',`addip` = '" . ip_address() . "',`uptime` = '" . time() . "',`upip` = '" . ip_address() . "',`lasttime` = '" . time() . "',`lastip` = '" . ip_address() . "'";

	    if (isset($_GET['invite']) || isset($_COOKIE['invite'])) {
		    if (isset($_GET['invite'])) {
			    $sql .= ",`invite_userid`={$_GET['invite']}";
		    } else {
			    $sql .= ",`invite_userid`={$_COOKIE['invite']}";
		    }
	    }
    ...
    public static function validBankNum($user_id)
    {
        global $mysql;
        $sql = "SELECT * FROM {account_bank}
    ...
    function QianDao($data)
    {
        ...
        $_tomorrow_time = strtotime(date("Y-m-d", strtotime('1 day')));
        $_yesterday_time = strtotime(date("Y-m-d", strtotime('-1 day')));

        $_select_sql = $_sql . " and addtime >= '$_today_time' and addtime < '$_tomorrow_time'";
        $sql = "select * from `{integral_log}` {$_select_sql} ";
        $result = $mysql->db_fetch_array($sql);
        if (!empty($result)) {
            return '今天已签到过,请明天再来';
        }
        ...
        //获取签到天数
        $_select_sql = $_sql . " and addtime >= '$_yesterday_time' and addtime < '$_today_time'";
        $sql = "select day from `{integral_log}` {$_select_sql}";
        $result = $mysql->db_fetch_array($sql);
        ...
        if (empty($result)) {
            ...
            $qiandao_day = $result['day'] + 1;
            if ($qiandao_day > 7) {
                $qiandao_integral = $qiandao_day % 7;
                if ($qiandao_integral == 0) {
                    $qiandao_integral = 7;
                }
            } else {
                $qiandao_integral = $qiandao_day;
            }
        }

        $sql = "insert into `{integral_log}` (`user_id`,`type_id`,`value`,`remark`,`op_user`,`addtime`,`addip`,`day`) values ({$data['user_id']},2,$qiandao_integral,'签到积分$qiandao_integral',0,'" . time() . "','" . ip_address() . "','$qiandao_day')";
        $db_reslut = $mysql->db_query($sql);
        if (empty($db_reslut)) {
            return '签到失败，请重试';
        } else {
            $insert_id = mysql_insert_id();
            //更新用户积分
            $sql = "update `{user}` set integral=integral+$qiandao_integral where user_id='{$data['user_id']}'";
            $db_reslut = $mysql->db_query($sql);
            //返回积分记录结果
            $sql = "select value,day from `{integral_log}` where id=$insert_id limit 1";
            $db_result = $mysql->db_fetch_array($sql);
            return $db_result;
        }
    }
}
