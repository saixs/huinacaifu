...
function check_rank($purview){
	...
	if (in_array("other_all",$admin_purview) || $_G['user_result']['type_id'] == 1){
        ...
        //echo 222;
		echo "<script>alert('你的权限将受到限制，请在指定的范围职责内操作!');location.href='/index.php?{$_G['con_houtai']}';</script>";exit;
	}
    die;
...
function RegEmailMsg($data = array()){
	...
	<div style="padding: 10px 0px; background: url(http://'.$_SERVER['HTTP_HOST'].'/data/images/base/email_button.png)  no-repeat ">
		<h1 style="padding: 0px 15px; margin: 0px; overflow: hidden; height: 48px;">
			...
			<img style="border-width: 0px; padding: 0px; margin: 0px;" alt="'.$webname.'用户中心" src="http://'.$_SERVER['HTTP_HOST'].'/data/images/base/email_logo.jpg" height="48" width="208">		</a>
		</h1>
		...
		<div style="padding: 2px 20px 30px;">
			...
			<p><br>此为自动发送邮件，请勿直接回复！如您有任何疑问，请点击<a title="点击联系我们" style="color: rgb(15, 136, 221);" href="http://'.$_SERVER['HTTP_HOST'].'/lianxi/main.html" target="_blank" >联系我们&gt;&gt;</a></p>
		</div>
	</div>
</div>
		';
...
function show_pages($data = array()){
	...
	//第一页
	if($page==1){
		$display .= ' <span><a>首页</a></span>';
	}else{
		if ($ajax == false) {
			$display .= " <span><a href='{$first_url}' class='wxy'>首页</a></span>";
		} else {
			$display .= " <span><a href='#' onclick='ajaxUrl(\"{$first_url}&ajaxInvest=true\")' class='wxy'>首页</a></span>";
		}
	}
	
	//上一页
	if($page==1){
		$display .= ' <span><a title="上一页">上一页</a></span>';
	}else{
		if ($ajax == false) {
			$display .= "<span><a href='{$up_url}'>上一页</a></span>";
		} else {
			$display .= "<span><a href='#' onclick='ajaxUrl(\"{$up_url}&ajaxInvest=true\")'>上一页</a></span>";
		}
	...
	//下一页
	if($page==$page_num){
		$display .= ' <span><a  title="下一页">下一页</a></span>';
	}else{
		if ($ajax == false) {
			$display .= " <span><a href='{$down_url}' >下一页</a></span>";
		} else {
			$display .= " <span><a href='#' onclick='ajaxUrl(\"{$down_url}&ajaxInvest=true\")'>下一页</a></span>";
		}
	}
	
	//最后一页
	if($page==$page_num){
		$display .= ' <span><a title="最后一页">尾页</a></span>';
	}else{
		if ($ajax == false) {
			$display .= " <span><a href='{$last_url}' >尾页</a><span>";
		} else {
			$display .= " <span><a href='#' onclick='ajaxUrl(\"{$last_url}&ajaxInvest=true\")' >尾页</a><span>";
		}
	}
	/*$display .=' <span>转到<input type="text" id="page_text" size="4" onkeydown="if (event.keyCode==13){location.href =\'index\'+this.value+\'.html\'+\''.$action.'\'}"  value="'.$page.'"  onfocus="this.select()" />页</span>';*/
	return $display.'</div>';
}
