...
class siteClass {
	...
	public static function GetMenu($data = array()){
		...
		if (is_array($result)){
			foreach ($result as $key => $value){
				if ($value['pid']==0 && $value['code'] == 'article'){
					$control_menu[$key] = $value;
					foreach ($result as $_key => $_value){
						if ($_value['pid']==$value['site_id'] && $value['code'] == 'article'){
							$control_menu[$key]['sub'][$_key] = $_value;
							foreach ($result as $__key => $__value){
								if ($__value['pid']==$_value['site_id'] && $value['code'] == 'article'){
